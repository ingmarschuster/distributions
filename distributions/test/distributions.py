...
def test_dirichlet():
    assert(np.abs(exp(dirichlet([5,1]).logpdf([0.5, 0.5])) - 0.3125) < 10**-3)
    assert(np.abs(exp(dirichlet([50,50,30]).logpdf([0.5,0.4,0.1])) - 0.00855995) < 10**-5)
    
    params = np.array((0.5, 0.3,0.1,0.1))
    assert(np.abs(exp(dirichlet(params).logpdf(params)) - 0.43189166) < 10**-5)
